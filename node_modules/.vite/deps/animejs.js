...
var Draggable = class {
	...
	*/
	...
	handleHover() {
		if (this.isFinePointer && this.cursor && !this.cursorStyles) this.cursorStyles = set(this.$trigger, { cursor: /** @type {DraggableCursorParams} */ this.cursor.onHover });
	}
	/**
	* @param  {Number} [duration]
	* @param  {Number} [gap]
	...
	*/
	handleDown(e) {
		...
		if (this.isFinePointer && this.cursor) this.bodyStyles = set(doc.body, { cursor: /** @type {DraggableCursorParams} */ this.cursor.onGrab });
	...
	handleEvent(e) {
		switch (e.type) {
			...
			case "mouseenter":
				this.handleHover();
				break;
			case "selectstart": preventDefault(e);
		}
	}
};
/**
	...
	*/
	constructor(targets, params) {
		...
		if (isNil(transformsPropertiesRegistered)) {
			if (isBrowser && (isUnd(CSS) || !Object.hasOwnProperty.call(CSS, "registerProperty"))) transformsPropertiesRegistered = false;
			else {
		...
		parsedTargets.forEach(($el, i) => {
			...
			for (let name in params) {
				...
				if (isObj(propertyValue)) {
					...
					if (!isUnd(from)) {
						if (!individualTransformProperty) elStyle[name] = keyframes[name][0];
						else {
...
function splitPropertiesFromParams(params) {
	...
	for (let name in params) {
		...
		if (name === "duration" || name === "delay" || isEase) {
			if (isEase) parameters[name] = value;
